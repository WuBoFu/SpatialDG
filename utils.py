...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='emb', random_seed=2020):
    ...
    emb = np.array(adata.obsm[used_obsm], dtype=np.float64)
    print("emb shape:", emb.shape)
    print("emb dtype:", emb.dtype)
    print("num_cluster:", num_cluster)
    res = rmclust(rpy2.robjects.numpy2ri.numpy2rpy(emb), num_cluster, modelNames)
    print("Mclust result:", res)
    if len(res) < 2 or res[-2] is None:
        raise ValueError("Mclust failed, check input data and cluster number.")
